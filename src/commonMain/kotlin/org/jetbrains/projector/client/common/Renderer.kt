...
class Renderer(private val renderingSurface: RenderingSurface) {
  ...
  private fun applyFont(newFont: Font, size: Double) {
    ctx.setFont(newFont, size)
    ...
    requestedState.font.let { requestedFont ->
      canvasState.font.let { currentFont ->
        if (currentFont != requestedFont) {
          requestedState.fontSize.let { requestedFontSize ->
            canvasState.fontSize.let { currentFontSize ->
              if (currentFont != requestedFont) {
                applyFont(requestedFont, requestedFontSize)
              }
            }
          }
        }
